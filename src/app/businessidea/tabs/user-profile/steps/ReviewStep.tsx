  ...
  return (
    <div className="space-y-4">
      <div className="p-3 rounded-lg border border-gray-200">
        <div className="text-sm font-semibold text-gray-900 mb-2">Profile</div>
        <SummaryRow label="Role" value={role} />
      </div>

      {rd?.role === Role.Student && (
        <div className="p-3 rounded-lg border border-gray-200">
          <div className="text-sm font-semibold text-gray-900 mb-2">Student</div>
      ...
      {rd?.role === Role.Professional && (
        <div className="p-3 rounded-lg border border-gray-200">
          <div className="text-sm font-semibold text-gray-900 mb-2">Professional</div>
      ...
      {rd?.role === Role.BusinessOwner && (
        <div className="p-3 rounded-lg border border-gray-200">
          <div className="text-sm font-semibold text-gray-900 mb-2">Business Owner</div>
      ...
      {rd?.role === Role.CareerShifter && (
        <div className="p-3 rounded-lg border border-gray-200">
          <div className="text-sm font-semibold text-gray-900 mb-2">Unemployed/Career Shifter</div>
      ...
      <div className="p-3 rounded-lg border border-gray-200">
        <div className="text-sm font-semibold text-gray-900 mb-2">Preferences</div>
      ...
      <div className="p-3 rounded-lg border border-gray-200">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {(data.skills || []).map((s) => (
            <span key={s} className="px-2.5 py-1 rounded-full text-xs bg-gray-100 text-gray-800 border border-gray-200">{s}</span>
          ))}
        </div>
      </div>

      <div className="flex items-center gap-3">
        <button type="button" disabled className="px-3 py-2 rounded-md text-sm bg-gray-300 text-gray-600 cursor-not-allowed" title="UI placeholder — backend not implemented yet">Save Profile</button>
        <button type="button" disabled className="px-3 py-2 rounded-md text-sm bg-gray-300 text-gray-600 cursor-not-allowed" title="UI placeholder — backend not implemented yet">Continue</button>
      </div>
