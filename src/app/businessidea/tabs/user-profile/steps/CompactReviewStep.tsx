      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
        ...
        >
          {(profileData.skills || []).length === 0 ? (
            <div className="text-[10px] text-gray-500 py-1">No skills selected</div>
          ) : (
            <div className="flex flex-wrap gap-1 pt-0.5">
              {(profileData.skills || []).slice(0, 5).map((s) => (
                <span key={s} className="px-1.5 py-0.5 rounded-full text-[9px] bg-indigo-50 text-indigo-700 border border-indigo-100">{s}</span>
              ))}
